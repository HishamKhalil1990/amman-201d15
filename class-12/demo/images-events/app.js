...
function renderTwoImages() {
  ...
  GoatImage.allImages[leftImageIndex].shown++;

  rightImageElement.src = GoatImage.allImages[rightImageIndex].source;
  GoatImage.allImages[rightImageIndex].shown++;
...
function handleUserClick(event) {
  ...
  if (userAttemptsCounter <= maxAttempts) {
    ...
    // let goatResult;
    // for(let i=0;i<GoatImage.allImages.length;i++){
    ...
    for (let i = 0; i < GoatImage.allImages.length; i++) {
      // 0
      goatVotes.push(GoatImage.allImages[i].votes);

      goatShown.push(GoatImage.allImages[i].shown);
    }
    viewChart();
